...
export const questionRouter = s.router(questionContract, {
    ...
    return {
      ...
      body: {
        data: questions.map(question => ({
          ...
          options: JSON.parse(question.options) as { text: string; is_correct: boolean }[],
  ...
  createQuestion: async ({ body, params }) => {
  ...
  updateQuestion: async ({ params, body }) => {
    ...
    const updatedQuestion = await db.question.update({
      ...
      data: {
        ...
        correct_option: body.correct_option,
        explanation: body.explanation,
        options: JSON.stringify(body.options.map((option: { text: string; is_correct: boolean }) => ({
          text: option.text,
    ...
    return {
      ...
      body: {
        data: {
          ...updatedQuestion,
          options: JSON.parse(updatedQuestion.options) as { text: string; is_correct: boolean }[],
        },
        isSuccess: true,
